...
MainWindow (mainwindow.*) → orchestrator: owns all widgets, routes signals/slots — frameless window with toolbar-as-title-bar, window drag/resize via nativeEvent & event()
  ...
  ├── CompilerErrorParser → header-only namespace, parses g++/MSVC compile errors and Python tracebacks → QList<SmdDiagnostic>
  ├── BottomPanel         → unified bottom panel (right splitter bottom) with two tabs: OutputPanel (terminal stdout/stderr) + DiagnosticsTab (error/warning list by DiagnosticSection). Manages provider connection lifecycle on tab switch. MD code block diagnostics injected on ▶ Run.
...
- **Preview code block run**: marked renderer adds ▶ button on code blocks with `data-block-index` attribute. Click stores `{lang, code, blockIndex}` in JS `_runCodeData` → navigates `runblock:execute` → C++ intercepts → saves temp file → ProcessRunner compiles/runs. stderr buffered for diagnostics parsing on finish. Manual stop (Ctrl+C) skips diagnostic parsing.
...
- **Diagnostics**: Two panels — `SmdDiagnosticsPanel` (SMD, per-cell, in SmdEditor splitter) and `BottomPanel` DiagnosticsTab (standalone `.cpp`/`.py` + MD code blocks, in bottom splitter). All `CompletionProvider` subclasses emit `diagnosticsUpdated(QList<SmdDiagnostic>)`. C++: clangd `textDocument/publishDiagnostics` → `CppCompletionProvider::parseDiagnostics()`. Python: Jedi `diagnostics` action (base64-encoded code, 500ms debounce on text change). `CodeEditor` caches diagnostics via `setDiagnostics()` → `m_diagnostics`; `MainWindow::currentChanged` restores from cache on tab switch. Non-code-file tabs auto-hide BottomPanel; code-file tabs reconnect provider and load cached diagnostics.
- **MD Code Block Diagnostics**: `.md` preview code block execution produces diagnostics via `CompilerErrorParser` (g++/MSVC for C++, traceback for Python). stderr buffered in `m_mdStderrBuffer`; parsed on compile/run finish. Results shown in BottomPanel DiagnosticsTab + preview squiggly lines (JS `applyBlockDiagnostics`). Diagnostics clear immediately on new run; manual stop (Ctrl+C) skips parsing. Content snapshots (`m_blockDiagnosticCode`) prevent stale line numbers on preview refresh. No Ctrl+D shortcut — only via bottom panel 诊断 tab. Switching from `.md` auto-closes panel.
