...
### 修复 v0.4.14
- **缩放重置行为修复**：`Ctrl+0` 或工具栏"重置"按钮现在将缩放恢复到**设置面板中配置的默认字体大小**，而非固定 100%。若用户在设置面板中将默认字体大小设为 150%，则缩放重置后所有编辑器应用 150% 缩放。
- **设置面板滚动区域背景修复**：修复设置面板中 `QScrollArea` 背景透明导致的内容区域与面板整体背景不一致的问题。通过设置 `QPalette::Window` 背景色、滚动区域及内部内容控件的显式背景色（`#2b2b2b`），确保深色主题下整个面板背景一致。
