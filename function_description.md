...
### 修复 v0.6.2
- SMD 单元格自适应高度：单元格高度始终由内容驱动，无最大行数限制，所有内容完整显示，彻底取消块内滚动条。
- 修复 `QTextDocument` 默认边距导致换行时首行被顶出可视区的问题。单元格容器布局末尾添加弹性伸缩项，防止单元格被拉伸至填满整个页面。编辑时光标自动跟随，连续换行时滚动区域同步下移以保持光标可见。
