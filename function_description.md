...
### 新增/优化 v0.0.10
- 历史数据在程序正常退出时保存到 config.ini，运行过程中不会频繁保存。`addFile(const QString &filePath)`不再提供持久化。
- 优化历史记录面板更新逻辑，降低性能开销。
- 新增历史记录删除功能。
- 历史记录面板打开时，点击面板外的任意区域，自动隐藏面板。
- 历史记录面板支持快捷键 `Ctrl+H`：按下可切换面板的显示/隐藏状态。
...
- 支持以下快捷键：
  ...
  - `Ctrl+=` 放大字体、`Ctrl+-` 缩小字体、`Ctrl+0` 重置缩放、`Ctrl+H` 打开/关闭历史记录面板
