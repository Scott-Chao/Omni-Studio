...
### 新增 v0.10.8
- **Markdown 编辑器缩进系统**：Tab 键插入空格缩进（默认 2 空格），不再插入字面 `\t` 字符导致过大的显示宽度
- **Markdown 换行自动缩进**：按下 Enter 换行时，新行自动保留上一行的前导空白，光标定位至上一行首个非空格字符的列位置
- **Markdown 智能退格**：在行首空白区域按 Backspace 时，一次删除一个缩进级别的空格（缩进宽度可配置），而非逐字符删除
- **代码/MD 缩进宽度独立设置**：设置面板编辑器中新增"MD 缩进宽度"选项，代码文件默认 4 空格，Markdown 文件默认 2 空格，独立调整互不影响
