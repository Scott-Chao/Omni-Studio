...
### 修复 v0.4.9
预览代码块语法高亮完善：修复 Python 三引号字符串（`"""..."""` / `'''...'''`）在预览中未正确渲染为注释的问题，同时补全 C++ 多行注释（`/* ... */`）的预览高亮。采用预处理方案：在逐行着色前提取多行文本块并以占位符替换，高亮完成后恢复为带注释颜色的 HTML 片段。

### 修复 v0.4.10
修复提交结果面板关闭后运行面板无法打开的问题：当 OpenJudge 提交结果面板（`SubmitResultPanel`）被插入右侧分割区（`m_rightSplitter`）后，分割器从 2 个控件变为 3 个控件（编辑器 + 提交结果面板 + 输出面板）。但 `showOutputPanel()` 和 `onSubmissionResultReady()` 调用 `setSizes()` 时始终只传 2 个值，导致在 3 控件场景下尺寸分配错位——提交结果面板获得了本应给输出面板的高度，而输出面板获得 0 像素。修复方案：两处 `setSizes()` 调用改为动态遍历分割器中的所有控件，精确匹配目标控件并分配对应高度，其余非目标控件设 0。
