...
### 新增
- 选中的文本区域在点击其他单元格或进入命令模式之后自动取消选中。
- **诊断悬停提示**：鼠标悬停在 SMD 代码单元格的错误/警告波浪线上时，延迟 400ms 后显示带颜色区分的诊断信息弹窗——错误使用红色背景（`#5a1d1d` + `#F44747` 边框），警告使用黄色背景（`#5a4d00` + `#CCA700` 边框），与波浪线颜色一致。弹窗在鼠标离开、点击或滚轮时自动关闭。该功能通过扩展 `HoverManager` 实现，诊断信息优先于 LSP 悬停提示显示，无需 LSP 网络往返（数据本地直接读取）。
- **诊断信息面板**：SMD 编辑器中新增诊断信息面板，快捷键 `Ctrl+E`（仅编辑模式下生效，命令模式下 `Ctrl+E` 恢复为导出 PDF 功能）。面板嵌入在编辑器底部（`SmdDiagnosticsPanel`），最大高度 200px，包含：
  - **可收起列表**：错误（Errors）和警告（Warnings）分组显示，默认展开，带有 ▼/▶ 三角指示器和计数（如"▼ Errors (3)"）。
  - **行号定位**：每个条目显示"Line N: 消息内容"，左侧带红色/黄色 2px 竖线强调。点击条目自动跳转到对应单元格的指定行。
