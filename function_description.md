...
### 新增 v0.10.9
- **文件树工具栏**：面包屑路径栏下方新增工具栏，左侧显示当前根目录文件夹名称，右侧预留操作按钮区域
- **工具栏标题省略**：当文件树面板宽度较窄导致标题与按钮重叠时，优先保证按钮完整显示，标题自动省略为 `...`（中间省略）
- **刷新文件树按钮**：工具栏右侧新增刷新按钮（旋转箭头图标），点击可强制重新读取当前目录并刷新文件树，适用于外部程序修改目录后自动监听失效的场景
