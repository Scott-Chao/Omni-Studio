...
### 新增/优化 v0.0.11
- 文件树右键菜单：“新建文件”与“新建文件夹”改为内联创建并立即进入重命名，不再弹出对话框。
  - 新建文件默认为 `.md`，名称为“新建文件.md”（冲突时自动编号为“新建文件(1).md”等）。
  - 新建文件夹默认名称为“新建文件夹”（冲突时自动编号为“新建文件夹(1)”等）。
- 重命名文件时，编辑框默认只选中文件主名，不选中扩展名；重命名文件夹时仍选中整个名称。
- 文件树始终保持“文件夹在上、文件在下”的排序（同类内按名称升序），新建或重命名后自动重新排序。
- 删除操作支持快捷键：
  - 文件树中选中项后直接按 `Delete` 键可触发删除（非重命名状态）。
  - 右键菜单打开后，按 `Delete` 键也可触发删除。
- 清理了不再使用的旧版对话框创建文件夹逻辑。
