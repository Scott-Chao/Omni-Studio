...
### 新增
- **LSP 异步启动**：C++/Python LSP 服务器启动改为完全异步，移除 `QProcess::waitForStarted(5000)` 阻塞调用。`LspClient::start()` 改为 `void` 返回，通过 `serverStarted` / `serverError` 信号通知启动结果；`CppCompletionProvider`、`PythonCompletionProvider`、`SmdLspManager` 均适配异步启动。`FailedToStart` 错误单独处理，不影响运行时错误的自动重启逻辑。
- **诊断悬停提示**：鼠标悬停在 SMD 代码单元格的错误/警告波浪线上时，延迟 400ms 后显示带颜色区分的诊断信息弹窗——错误使用红色背景（`#5a1d1d` + `#F44747` 边框），警告使用黄色背景（`#5a4d00` + `#CCA700` 边框），与波浪线颜色一致。弹窗通过临时设置编辑器 `QToolTip` stylesheet 消除黑边，恢复时自动还原原样式。诊断信息优先于 LSP 悬停提示显示，无需 LSP 网络往返（数据本地直接读取）。
- **诊断信息面板**：SMD 编辑器中新增诊断信息面板（`SmdDiagnosticsPanel`），快捷键 `Ctrl+E`（仅编辑模式下生效，命令模式下 `Ctrl+E` 恢复为导出 PDF 功能）。面板嵌入在编辑器底部的 `QSplitter` 中，用户可拖动上边缘自由调节高度，初始高度约为视图的 1/3（最低 100px）：
  - **可收起列表**：错误和警告分组显示，点击表头展开/收起（通过 `<a>` 标签使 `linkActivated` 信号可触发）。三角指示器使用视觉对等的 ▾/▸ 字符，默认展开。空分组自动隐藏表头，仅显示有诊断的类型；两种类型均无时显示"无诊断信息"。
  - **行号定位**：每个条目显示"行 N: 消息内容"，左侧带红色/黄色 2px 竖线强调。点击条目自动跳转到对应单元格的指定行。
  - **延迟刷新**：内容变化后延迟 500ms 自动刷新，避免快速打字时频繁重建列表。
  - **自动开关**：切换至命令模式（Esc）时自动关闭面板；切换单元格时自动刷新当前单元格的诊断信息；关闭/切换文件时面板自动清空。面板提供手动关闭按钮（✕）。
  - **数据来源**：通过 `SmdLspManager::diagnosticsForCell()` 读取当前活动单元格的诊断信息（clangd 提供的 C++ 诊断）。面板本身不存储数据，始终从 LSP 管理器实时读取。
