...
- 面包屑路径栏：文件树顶部展示当前根目录的完整路径，每个文件夹段可点击快速跳转。路径自动换行不撑宽左侧面板，根目录切换时同步更新。其下方为文件树工具栏，显示当前文件夹名称及操作按钮。工具栏右侧新增"新建文件"和"新建文件夹"按钮（文件/文件夹图标右下角带加号 badge），置于折叠/刷新按钮左侧，功能与右键菜单完全一致，点击后内联创建并进入重命名状态，图标颜色随主题自动更新。
...
### 新增 v0.13.30
- 文件树工具栏新增"新建文件"和"新建文件夹"按钮（文件/文件夹图标右下角带加号 badge），置于折叠/刷新按钮左侧。点击在当前目录或选中目录内联创建 `.md` 文件/文件夹并进入重命名状态，功能与右键菜单完全一致。两个 SVG 图标通过 `coloredSvgIcon()` 实现主题色自动更新。
