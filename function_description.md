...
### 新增 v0.6.1
- **保存对话框增加 .smd 格式支持**：所有编辑模式的"另存为"对话框中均加入 `Smart Markdown文件 (*.smd)` 类型选项，新建文件可方便地保存为 `.smd` 格式。默认选中 `Markdown文件 (*.md)` 类型保持原有行为。
- **新建 .smd 文件即时显示内容**：从 Markdown 编辑模式另存为 `.smd` 文件后，自动切换到单元格编辑模式，解析已有内容生成单元格并立即可见，无需重新打开文件。
- **SMD 单元格自适应高度**：单元格容器布局末尾添加弹性伸缩项，防止单元格被拉伸至填满整个页面。单元格高度始终由内容驱动（初始 1 行高度），仅在内容超出最大限制（约 40 行）时显示滚动条。
