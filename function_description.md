...
### 设置面板完善 v0.13.2
- **设置面板重构**：合并设置面板页面，从单页改为侧边栏导航 + 多页面布局（常规/外观/AI 服务/快捷键/工具）
- **外观页颜色控件**：6 个可折叠区域共 30 色，主题色实时调整预览
- **AI 服务页增强**：Temperature 滑块、API Key 密码切换、提示词防抖、OJ 密码切换
- **快捷键页视觉分组**：添加分组标签（通用/CodeEditor/SmdEditor/SmdEditor 命令模式/输出面板），减少查找时间
- **快捷键冲突 Minecraft 化**：取消冲突弹窗，改为允许冲突并以红色文字标出，由用户自行决定是否修改
- **工具页**：ConfigManager 新增 tools 段，四个 Provider 支持可配置的工具路径
- **Bug 修复**：颜色覆盖值未同步到 ThemeManager 且重置后面板 UI 不刷新；QSpinBox 内部 QLineEdit 高度膨胀导致标签文本错位；修复括号匹配错误
