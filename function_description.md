...
### 修复 v0.13.5
- 修复大纲面板浅色主题下文字颜色的问题：文字颜色改为主题自适应（`sideBar.foreground` 与 `sideBar.background` 混合），不再硬编码深色主题色值
- 修复大纲面板缩进问题：缩进改为相对于文件中最小标题层级，避免顶层标题过度右移
- 修复大纲面板长标题截断问题：添加自定义 `ElideDelegate`，超出边界时显示 "…" 省略号而非直接截断
