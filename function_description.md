...
### 新增 v0.10.4
帮助系统
- 新增帮助面板（HelpPanel），工具栏添加帮助按钮（位于 spacer 与面板按钮之间），快捷键 `F1`
- 帮助面板以悬浮式 overlay 覆盖在主窗口上方，半透明遮罩层居中显示，支持标题栏拖拽移动
- 左侧 170px 分类列表（QListWidget），包含 14 个模块分类：欢迎使用、文件管理、编辑器、Markdown 预览、SMD 编辑器、代码编译运行、搜索与大纲、双向链接与标签、历史记录、评测系统、OpenJudge 集成、AI 助手、设置与快捷键、快捷键参考
- 右侧 QTextBrowser 加载 HTML 资源文件（`resources/help-content.html`），深色主题展示各模块详细说明
- 滚动浏览时左侧分类自动同步高亮（基于 QTextDocument 的 pixel Y 位置匹配），点击分类名自动滚动到对应章节
- 点击遮罩层背景区域或关闭按钮关闭面板
