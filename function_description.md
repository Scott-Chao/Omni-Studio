...
### 新增 v0.10.1
快捷键自定义
- **快捷键面板交互式录制**：设置面板快捷键页面从只读表格改造为交互式 KeyRecorder 控件，支持点击录制、组合键捕获、Delete/Backspace 清除、Escape 取消
- **冲突检测**：录制新快捷键时自动检测冲突，弹窗提示覆盖或取消；覆盖时自动清空冲突操作的快捷键并同步 QAction
- **即时生效**：快捷键修改后立即刷新对应 QAction 的 Shortcut，无需重启即可生效
- **持久化存储**：自定义快捷键通过 SettingsManager 的 override 机制保存至 config.ini，重启后自动恢复
- **恢复默认**：支持一键重置所有快捷键至出厂配置
