...
### 修复 v0.2.18
修复运行时粘贴多行输入导致进程异常与输入输出不同步问题
- **问题 1**：`pasteToInput()` 逐行发送粘贴内容，每行间调用 `processEvents()` 让位于事件循环。导致三个问题：
  - 末行丢失：剪贴板文本无尾随 `\n` 时，最后一行仅缓冲不发送，进程永远等不到最后一个输入。
  - 重入崩溃：`processEvents()` 期间进程可能已完成并触发 `setRunning(false)`，后续循环迭代在只读编辑器中插入文本，或向已退出进程写入数据，触发 `QProcess::ReadError`（代码 4）。
  - 输入输出交错：`processEvents()` 允许进程输出在输入回显之间到达，导致终端显示混乱。
- **修复 1**：重写 `pasteToInput()`，将粘贴内容作为整体一次性发送。
  - 移除逐行分割与 `processEvents()` 循环，消除重入风险。
  - 统一回显粘贴文本后再发送至进程，避免输入输出交错。
  - 末行由 `writeInput` 自动追加 `\n` 保证终止，不再丢失。
  - 粘贴后清空 `m_inputBuffer`，避免与之前的手动输入残留混淆。
- **问题 2**：粘贴回显末尾缺少换行符，导致回显的最后一行与进程输出的第一行视觉拼接（如 `101---`）。
- **修复 2**：`pasteToInput()` 回显后检测文本是否以 `\n` 结尾，若无则补插一个 `\n`，保证回显与输出分行显示。
- **问题 3**：C++ 文件直接运行（F7）时 `onRun()` 未调用 `clearOutput()`（Python 路径和 `onCompileAndRun()` 均有此调用），导致前次运行的输出残留，看起来输出重复出现。
- **修复 3**：`MainWindow::onRun()` 在 C++ 路径中新增 `m_outputPanel->clearOutput()` 调用，与 Python 路径保持一致。
