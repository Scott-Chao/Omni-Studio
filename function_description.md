...
### 新增 v0.5.0
分屏预览模式：Markdown 分屏预览功能
- 编辑器区域被可拖拽竖直分隔条分为左右两部分——左侧源码，右侧渲染预览
- 工具栏新增"分屏预览"按钮（可勾选），快捷键 `Ctrl+P`。与全屏预览互斥，切换文件时记忆状态。
- 右侧预览通过防抖机制（500ms）延迟刷新，仅在文本变化时更新。
- 复用现有预览模板和信号链路，wikilink 点击、tag 点击、代码块运行等交互功能与全屏预览一致。
