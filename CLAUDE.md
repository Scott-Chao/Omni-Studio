...
- **Local Judge**: `JudgePanel::runAllRequested` → `MainWindow::onJudgeRunAll` (saves code, shows dock, validates) → `JudgePanel::runJudge(filePath)` → `JudgeEngine::start()` discovers `.in`/`.out` pairs → compile → runs each test case sequentially with 1000ms timeout + memory monitoring via `GetProcessMemoryInfo`. Output trimmed line-by-line comparison. Results in table: AC (green), WA (red), TLE (blue), MLE (purple), RE (orange). Click failed row for expected vs actual diff.
...
**JudgeEngine** (`judgeengine.h/cpp`) — `QObject` managing the local judge pipeline. `discoverTests()` scans folder for matching `.in`/`.out` pairs. `start()` compiles via `CompilerUtils` spawns `QProcess`, then runs each test sequentially with 1000ms timeout (`QTimer`) and memory monitoring (100ms `QTimer` polling `GetProcessMemoryInfo(PeakWorkingSetSize)` via `OpenProcess` on child PID). Output comparison strips trailing blank lines then `.trimmed()` line-by-line. Status codes: AC (match), WA (mismatch), RE (crash/nonzero exit), TLE (timeout), MLE (peak > 65536KB). `m_testHandled` flag prevents dual-fire between timeout and process finish. Emits `compileFinished`, `testStarted`, `testFinished`, `allTestsFinished`.
