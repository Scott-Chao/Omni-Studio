...
- **MD Code Block Diagnostics**: `.md` files in preview mode can show diagnostics from code block execution. Each code block gets a `data-block-index` attribute. On ▶ Run click, the block index is passed through `_runCodeData` → `runCodeBlockRequested(lang, code, blockIndex)`. stderr is buffered during execution, then parsed by `CompilerErrorParser` (g++/MSVC format for C++, traceback for Python) into `QList<SmdDiagnostic>`. Results are shown in BottomPanel's DiagnosticsTab AND as wavy underlines in the preview via `window.applyBlockDiagnostics()`. Diagnostics clear immediately when a new run starts. Manual stop (Ctrl+C) skips parsing to avoid process-level errors being treated as code errors. On preview refresh, content snapshots (`m_blockDiagnosticCode`) are compared — only blocks with unchanged code keep their diagnostics, preventing squiggly-line drift. No keyboard shortcut; only accessible via the bottom panel's 诊断 tab. Switching away from `.md` auto-closes the bottom panel.
