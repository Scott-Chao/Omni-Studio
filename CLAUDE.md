...
**JudgePanel** (`judgepanel.h/cpp`) — `QDockWidget` content widget for judge UI. Folder selector (`QLineEdit` + browse button), 5-column `QTableWidget` (#, name, result, time, memory), detail `QPlainTextEdit` (shows expected vs actual on click), summary `QLabel`, run/stop buttons. Owns a `JudgeEngine` and connects all signals internally. Emits `runAllRequested()` for `MainWindow` to trigger. `runJudge(sourceFile)` sets source and test folder, starts engine.
