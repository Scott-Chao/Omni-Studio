...
MainWindow (mainwindow.*) → orchestrator: owns all widgets, routes signals/slots
  ...
  ├── OpenJudgeWindow     → QMainWindow singleton, browse OpenJudge homework→problems→detail, extract samples
  │   └── Crawler         → QNetworkAccessManager + QNetworkCookieJar, HTTP crawler for cxsjsx.openjudge.cn
  ├── LoginDialog         → QDialog, username/password for OpenJudge CSRF login
...
**JudgePanel** (`judgepanel.h/cpp`) — `QDockWidget` content widget for judge UI. Two-row folder selector (row 1: `QLineEdit` + "浏览..." button; row 2: "从Openjudge获取" button), 5-column `QTableWidget` (#, name, result, time, memory), detail `QPlainTextEdit` (shows expected vs actual on click), summary `QLabel`, run/stop buttons. Owns a `JudgeEngine` and connects all signals internally. Emits `runAllRequested()` for `MainWindow` to trigger, and `openJudgeRequested()` for OpenJudge window. `runJudge(sourceFile)` sets source and test folder, starts engine. `setTestFolder(path)` sets folder and clears results (used by OpenJudge integration).

**OpenJudgeWindow** (`openjudgewindow.h/cpp`) — Independent `QMainWindow` for browsing OpenJudge problems (dark theme `#2D2D30`). Toolbar: [选择此题目] [← 返回] [stretch] [刷新] [登录]. Three-state navigation via `OjViewState` enum: homework list (ongoing + past with pagination), problem list, problem detail (left section nav 100px + right `QTextBrowser`). Detail page uses `QHBoxLayout` with spacing=0 for no-gap layout. `extractSamples()` matches sections by "样例输入"/"样例输出" headings, extracts `<pre>` blocks via regex, pairs 1:1. `writeSamplesToCache()` writes `testN.in`/`testN.out` to `QStandardPaths::TempLocation + "/SM-OJ-<timestamp>"`. Emits `sampleSelected(folderPath)`. Managed as singleton via `QPointer<OpenJudgeWindow>` in `MainWindow`.

**Crawler** (`crawler.h/cpp`) — `QObject`-based HTTP crawler for `cxsjsx.openjudge.cn`. Uses `QNetworkAccessManager` + `QNetworkCookieJar`. CSRF login flow: GET `/login/` → extract token → POST credentials → GET homepage. `parseMainPage()` extracts ongoing (`<ul class="current-contest">`) and past (`<div class="past-contest">`) homework sections, plus "更多" pagination link. `fetchPastPage()` fetches `/contests/past` and parses contest links (filter by `hw`/`practise`/`midexam`/`pool`/`contest` keywords). `parseProblemDetail()` uses dual-strategy HTML parsing (`<dt>/<dd>` primary, `<h3>` fallback). Static helpers `decodeHtmlEntities()` and `stripHtmlTags()` are public for use by `OpenJudgeWindow`. Debug logging to `crawler_debug.log`. Data structs: `HomeworkItem`, `ProblemSection`, `ProblemDetail`, `PageInfo`.

**LoginDialog** (`logindialog.h/cpp`) — Simple `QDialog` with username/password fields and Login/Skip buttons.
